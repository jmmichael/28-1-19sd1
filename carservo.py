...
class App:
    def __init__(self, master):
       ...
       # scale = Scale(frame, from_=0, to=180,
        #      orient=VERTICAL, length=500, resolution=0.1, command=self.updatev)
       # scale.grid(column=0)
        #scale.set(90)
    ...
    #def updatev(self, angle):
     #   pulse_len = int(float(angle) * 500.0 / 180.0)# + 110
     #   pwm.setPWM(2, 0, pulse_len) #change 0 to point where it becomes active
        #pwm.setPWM(1, 0, pulse_len)

root = Tk()
root.wm_title('Servo Control')
app = App(root)
root.geometry("550x50+100+100") #w h
